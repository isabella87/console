...
public class BrowseBaPrjBorPersFrame
        extends InternalFramePane {
    private final Logger logger = LoggerFactory.getLogger(BrowseBaPrjBorPersFrame.class);

    /**
     * {@inheritDoc}
     */
    public BrowseBaPrjBorPersFrame() {
    ...
    private void search(
        ...
        params.put("start-create-time", startDate);
...
//串行异步，链式异步
    ...
    private void create(
        ...
        final EditBaPrjBorPerDlg dlg = new EditBaPrjBorPerDlg(0);
    ...
    private void edit(
        ...
        final long id = tableModel.getNumberByName(selectedRow, "bpmpId");
        final EditBaPrjBorPerDlg dlg = new EditBaPrjBorPerDlg(id);
        dlg.setFixedSize(false);
    ...
    private void delete(
        ...
        if (confirm(null, confirmDeleteText)) {
            ...
            final JTable table = controller().get(JTable.class, "list");
            final TypedTableModel tableModel = (TypedTableModel) table.getModel();
            final long bpeId = tableModel.getNumberByName(table.getSelectedRow(), "bpmpId");

            new BaPrjBorPersProxy().del(bpeId)
    ...
    private void check(
        ...
        final long id = tableModel.getNumberByName(selectRow, "bpmpId");
        final ChooseProtocolDlg dlg = new ChooseProtocolDlg(id, 28,1);
        dlg.setFixedSize(false);
        showModel(null, dlg);
    }
    ...
    private void creditFile(
        ...
        final long id = tableModel.getNumberByName(selectRow, "bpmpId");
        final ChooseProtocolDlg dlg = new ChooseProtocolDlg(id, 58,2);
        dlg.setFixedSize(false);
        showModel(null, dlg);
    }
    ...
    private void debtFile(
        ...
        final long id = tableModel.getNumberByName(selectRow, "bpmpId");
        final ChooseProtocolDlg dlg = new ChooseProtocolDlg(id, 60,2);
        dlg.setFixedSize(false);
        showModel(null, dlg);
    }
